...
public final class ARM_10 {
			...
			switch (instruction) {
				case 0x00000020 : // LDR{cond}H Rd, <Address>
					cpu.setRegister(rdIndex,
							memory.loadHalfWord(rnValue) & 0x0000FFFF);
					break;
	...
	final static String[] InstructionName = {"str%h", "ldr%h", "ldr%sb",
			"ldr%sh",};
	...
	public static String disassemble(ARM7TDMI cpu, MemoryInterface memory,
		...
		if ((opcode & 0x00400000) == 0) { // {+/-}Rm
			...
			int immediate = ((opcode & 0x00000F00) >>> 4)
					| (opcode & 0x0000000F);
		...
		return InstructionName[instruction].replace("%", cond) + " " + rd + ", "
				+ address;
	}
