...
public final class ARM_5 {
	...
	public static void execute(ARM7TDMI cpu, MemoryInterface memory,
		...
		if ((opcode & 0x02000000) != 0) { // operand2 is an immediate value
			...
			if (shiftAmount != 0) {
				...
				operand2 = (operand2 << (32 - shiftAmount))
			...
			if ((opcode & 0x00000010) == 0) { // Shift by immediate
				...
				switch (shiftType) {
					...
					case 0x00000040 : // ASR
						if (shiftAmount != 0) {
							...
							cFlag = ((operand2 & 0x80000000) != 0);
							operand2 >>= 31; // Fill all bits of operand2 with
												// bit 31
						}
						break;
				...
				shiftAmount = (rsValue + ((rsIndex == ARM7TDMI.PC) ? 4 : 0))
						& 0x000000FF;
		...
		switch (opcode & 0x01E00000) {
			...
			case 0x00600000 : // RSB{cond}{S} Rd, Rn, Op2
				...
				cpu.setRegister(rdIndex, rdValue);
				if (sBit)
					cpu.setVCFlagsForSUB(operand2, operand1, rdValue);
				break;
	...
	final static String[] InstructionName = {"and", "eor", "sub", "rsb", "add",
			"adc", "sbc", "rsc", "tst", "teq", "cmp", "cmn", "orr", "mov",
			"bic", "mvn",};

	final static String[] ShiftInstructionName = {"lsl", "lsr", "asr", "ror",};
	...
	public static String disassemble(ARM7TDMI cpu, MemoryInterface memory,
		...
		boolean isTestInstruction = ((instruction >= 0x08)
				&& (instruction <= 0x0B));
		boolean isMoveInstruction = ((instruction == 0x0D)
				|| (instruction == 0x0F));

		String sBit = (((opcode & 0x00100000) == 0) || isTestInstruction)
				? ""
				: "s";
		...
		if ((opcode & 0x02000000) != 0) {
			...
			int shiftAmount = (opcode & 0x00000F00) >>> 7;
			if (shiftAmount != 0) {
				immediate = (immediate << (32 - shiftAmount))
			...
			op2 = cpu.getRegisterName(opcode & 0x0000000F) + ", "
					+ ShiftInstructionName[shiftType];
		...
		return InstructionName[instruction] + cond + sBit + " "
				+ (isTestInstruction ? "" : (rd + ", "))
				+ (isMoveInstruction ? "" : (op1 + ", ")) + op2;
	}
