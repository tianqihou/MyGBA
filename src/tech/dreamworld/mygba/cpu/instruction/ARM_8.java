...
public final class ARM_8 {
	...
	public static void execute(ARM7TDMI cpu, MemoryInterface memory,
		...
		switch (opcode & 0x00600000) {
			case 0x00000000 : // UMULL{cond}{S} RdLo, RdHi, Rm, Rs
				rdValue = ((long) rmValue & 0xFFFFFFFFL)
						* ((long) rsValue & 0xFFFFFFFFL);
				break;
			...
			case 0x00200000 : // UMLAL{cond}{S} RdLo, RdHi, Rm, Rs
				...
				rdValue = ((long) rmValue & 0xFFFFFFFFL)
						* ((long) rsValue & 0xFFFFFFFFL);
				rdValue += ((long) rdhiValue << 32)
						| ((long) rdloValue & 0xFFFFFFFFL);
				break;
	...
	final static String[] InstructionName = {"umull", "umlal", "smull",
			"smlal",};
	...
	public static String disassemble(ARM7TDMI cpu, MemoryInterface memory,
		...
		return InstructionName[instruction] + cond + sBit + " " + rdlo + ", "
				+ rdhi + ", " + rm + ", " + rs;
	}
