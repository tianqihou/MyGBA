...
public final class THUMB_9 {
	...
	public static void execute(ARM7TDMI cpu, MemoryInterface memory,
		...
		switch (opcode & 0x1800) {
			case 0x0000 : // STR Rd, [Rb, #nn]
				memory.storeWord(rbValue + (offset << 2),
						cpu.getRegister(rdIndex));
				break;

			case 0x0800 : // LDR Rd, [Rb, #nn]
				cpu.setRegister(rdIndex,
						memory.loadWord(rbValue + (offset << 2)));
				break;

			case 0x1000 : // STRB Rd, [Rb, #nn]
				memory.storeByte(rbValue + offset,
						(byte) cpu.getRegister(rdIndex));
				break;

			case 0x1800 : // LDRB Rd, [Rb, #nn]
				cpu.setRegister(rdIndex,
						memory.loadByte(rbValue + offset) & 0x000000FF);
				break;
	...
	final static String[] InstructionName = {"str", "ldr", "strb", "ldrb",};
	...
	public static String disassemble(ARM7TDMI cpu, MemoryInterface memory,
		...
		return InstructionName[instruction] + " " + rd + ", [" + rb + ", "
				+ Hex.toHexString(off, Hex.Byte) + "]";
	}
