...
public final class THUMB_10 {
	...
	public static void execute(ARM7TDMI cpu, MemoryInterface memory,
		...
		if ((opcode & 0x0800) == 0) { // STRH Rd, [Rb, #nn]
			memory.storeHalfWord(rbValue + offset,
					(short) cpu.getRegister(rdIndex));
		} else { // LDRH Rd, [Rb, #nn]
			cpu.setRegister(rdIndex,
					memory.loadHalfWord(rbValue + offset) & 0x0000FFFF);
		}
	}

	final static String[] InstructionName = {"strh", "ldrh",};
