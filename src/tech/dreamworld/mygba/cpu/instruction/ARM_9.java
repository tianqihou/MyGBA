...
public final class ARM_9 {
	...
	public static void execute(ARM7TDMI cpu, MemoryInterface memory,
		...
		if (isWriteBack) { // Update Rn
			...
			if (rnIndex == ARM7TDMI.PC)
				cpu.flushARMPipeline();
		}
	}

	final static String[] InstructionName = {"str", "ldr",};
	...
	public static String disassemble(ARM7TDMI cpu, MemoryInterface memory,
		...
		if ((opcode & 0x02000000) == 0) { // <#{+/-}expression>
			address = "<" + sign
					+ Hex.toHexString(opcode & 0x00000FFF, Hex.Bit_12) + ">";
		} else { // {+/-}Rm{,<shift>}
			...
			if ((shiftType != 0) || (shiftAmount != 0)) { // Ignore lsl#0
				address += ", " + ShiftInstructionName[shiftType] + "#"
						+ shiftAmount;
			}
		}
		...
		return InstructionName[instruction] + cond + bBit + tBit + " " + rd
				+ ", " + address;
	}
