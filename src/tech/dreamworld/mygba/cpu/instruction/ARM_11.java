...
public final class ARM_11 {
	...
	public static void execute(ARM7TDMI cpu, MemoryInterface memory,
		...
		if ((opcode & 0x00100000) == 0) { // STM
			int mode = (isPSRBitSet) ? ARM7TDMI.USRMode : cpu.getMode();
			if (mode == ARM7TDMI.USRMode)
				System.out.println("Unsupported opcode (STM in User mode)");

			int i = 0;
			while (i < 15) {
				if ((opcode & (1 << i)) != 0) {
					memory.storeWord(stackAddress, cpu.getRegister(i)); // TODO:
																		// USR
																		// mode
					stackAddress += 4;
					break;
			...
			int mode = (isPSRBitSet && !isPCBitSet)
					? ARM7TDMI.USRMode
					: cpu.getMode();
			if (mode == ARM7TDMI.USRMode)
				System.out.println("Unsupported opcode (LDM in User mode)");
			...
			for (int i = 0; i < 15; i++) {
				if ((opcode & (1 << i)) != 0) {
					cpu.setRegister(i, memory.loadWord(stackAddress)); // TODO:
																		// USR
																		// mode
					stackAddress += 4;
				}
	...
	final static String[] InstructionName = {"stm", "ldm",};
	...
	public static String disassemble(ARM7TDMI cpu, MemoryInterface memory,
		...
		String amod = (((opcode & 0x00800000) == 0) ? "d" : "i")
				+ (((opcode & 0x01000000) == 0) ? "a" : "b");
		...
		for (byte i = 0; i <= 16; i++) {
			if ((regs & (1 << i)) == 0) {
				if (reg != -1) {
					...
					if (reg < last) {
						if (reg == (last - 1))
							rlist += "%" + cpu.getRegisterName(last);
						else
							rlist += "-" + cpu.getRegisterName(last);
					}
					reg = -1;
		...
		return InstructionName[instruction] + cond + amod + " " + rn + wBit
				+ ", " + "{" + rlist + "}" + sBit;
	}
