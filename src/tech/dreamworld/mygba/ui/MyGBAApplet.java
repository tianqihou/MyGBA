...
import tech.dreamworld.mygba.MyGBA;
import tech.dreamworld.mygba.gfx.GFXScreen;
import tech.dreamworld.mygba.memory.IORegMemory;
import tech.dreamworld.mygba.memory.Memory;

public final class MyGBAApplet extends JApplet
		implements
			ActionListener,
			KeyListener,
			MouseListener {
	...
	private MyGBAFileFilter fileFilter;

	private final static String OpenBIOSCommand = "OPEN_BIOS",
			OpenROMCommand = "OPEN_ROM", ResetCommand = "RESET",
			PauseCommand = "PAUSE", LaunchDebuggerCommand = "LAUNCH_DEBUGGER",
			DisplayAboutInfoCommand = "DISPLAY_ABOUT_INFO";

	private final static int OpenBIOSKey = KeyEvent.VK_F1,
			OpenROMKey = KeyEvent.VK_F2, ResetKey = KeyEvent.VK_R,
			PauseKey = KeyEvent.VK_P, LaunchDebuggerKey = KeyEvent.VK_D;

	public MyGBAApplet() {
		this(true);
	}

	public MyGBAApplet(boolean isApplet) {
	...
	public void init() {
		...
		memory = mygba.getMemory();
		iorMem = memory.getIORegMemory();

		gfxScreen = new GFXScreen(mygba.getGraphics());
		...
		debuggerMenuItem.setAccelerator(KeyStroke
				.getKeyStroke(LaunchDebuggerKey, KeyEvent.ALT_DOWN_MASK));
		...
		if (isApplet) {
			...
			fileFilter = new MyGBAFileFilter();
		...
		// try {
		// biosURL = new URL("file:/C:/BIOS.gba");
