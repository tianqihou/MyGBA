...
final class DebuggerDialog extends JDialog
		implements
			ActionListener,
			KeyListener {
	...
	private final static int RefreshKey = KeyEvent.VK_F5,
			StepKey = KeyEvent.VK_F7, CloseKey = KeyEvent.VK_ESCAPE;

	private final static String SwitchMemoryBankCommand = "SWITCH_MEM_BANK",
			RefreshCommand = "REFRESH", StepCommand = "STEP",
			CloseCommand = "CLOSE";

	public DebuggerDialog(MyGBA ygba) {
		cpu = ygba.getCPU();
		...
		TableColumnModel programViewerColumnModel = programViewerTable
		...
		TableColumnModel registerViewerColumnModel = registerViewerTable
				.getTableHeader().getColumnModel();
		...
		TableColumnModel flagViewerColumnModel = flagViewerTable
				.getTableHeader().getColumnModel();
		...
		refreshButton
				.setText("Refresh" + " " + KeyEvent.getKeyText(RefreshKey));
	...
	private void go(int offset) {
		...
		int y = memory.getInternalOffset(memoryBank,
				offset) >>> (cpu.getTFlag() ? 1 : 2);
