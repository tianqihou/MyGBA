...
import tech.dreamworld.mygba.memory.ObjectMemory;
import tech.dreamworld.mygba.memory.PaletteMemory;
import tech.dreamworld.mygba.memory.VideoMemory;

public final class GFX {

	public final static int XScreenSize = 240, YScreenSize = 160;
	...
	private void drawMode0Line(int yScreen) {
		...
		for (int p = 3; p >= 0; p--) {
			if (bg3Enabled && (bg3Priority == p))
				drawBGTextModeLine(yScreen, 3);
			if (bg2Enabled && (bg2Priority == p))
				drawBGTextModeLine(yScreen, 2);
			if (bg1Enabled && (bg1Priority == p))
				drawBGTextModeLine(yScreen, 1);
			if (bg0Enabled && (bg0Priority == p))
				drawBGTextModeLine(yScreen, 0);
			if (objEnabled)
				drawOBJLine(yScreen, p);
		}
	}
	...
	private void drawMode1Line(int yScreen) {
		...
		for (int p = 3; p >= 0; p--) {
			if (bg2Enabled & (bg2Priority == p))
				drawBGRotScalModeLine(yScreen, 2);
			if (bg1Enabled & (bg1Priority == p))
				drawBGTextModeLine(yScreen, 1);
			if (bg0Enabled & (bg0Priority == p))
				drawBGTextModeLine(yScreen, 0);
	...
	private void drawMode2Line(int yScreen) {
		...
		for (int p = 3; p >= 0; p--) {
			if (bg3Enabled & (bg3Priority == p))
				drawBGRotScalModeLine(yScreen, 3);
			if (bg2Enabled & (bg2Priority == p))
				drawBGRotScalModeLine(yScreen, 2);
			if (objEnabled)
				drawOBJLine(yScreen, p);
	...
	private void drawMode3Line(int yScreen) {
		...
		if (bg2Enabled) {
			...
			int y = (isMosaicEnabled
					? (yScreen - (yScreen % yMosaic))
					: yScreen);

			for (int xScreen = 0; xScreen < XScreenSize; xScreen++) {
				int x = (isMosaicEnabled
						? (xScreen - (xScreen % xMosaic))
						: xScreen);
	...
	private void drawMode4Line(int yScreen) {
		...
		if (bg2Enabled) {
			...
			for (int xScreen = 0; xScreen < XScreenSize; xScreen++) {
				...
				int colorIndex = vidMem
						.getByte(frameAddress + ((y * XScreenSize) + x)) & 0xFF;
	...
	private void drawBGTextModeLine(int yScreen, int bgNumber) {
		...
		for (int xScreen = 0; xScreen < XScreenSize; xScreen++) {
			...
			int tileDataOffset = ((yTileDataOffset + xTileDataOffset) * 2)
					+ (yOffsetToAdd + xOffsetToAdd);
			...
			if (is256ColorPalette) {
				int colorIndex = vidMem.getByte(characterBase
						+ (tileNumber * 8 * 8) + (tileY * 8) + (tileX)) & 0xFF;
				...
				int colorIndex = vidMem.getByte(characterBase
						+ (tileNumber * 8 * 4) + (tileY * 4) + (tileX / 2))
						& 0xFF;
				...
				if (colorIndex != 0) {
					...
					short rgb15 = palMem.getHalfWord(
							((paletteNumber * 16) + colorIndex) * 2);
					pixels[(yScreen * XScreenSize) + xScreen] = toRGBA(rgb15);
				}
			}
		}
	...
	private void drawBGRotScalModeLine(int yScreen, int bgNumber) {
		...
		for (int xScreen = 0; xScreen < XScreenSize; xScreen++) {
			...
			if ((x >= 0) && (x < xySize) && (y >= 0) && (y < xySize)) {
				...
				int tileNumber = vidMem.getByte(
						screenBase + (yTile * (xySize / 8)) + xTile) & 0xFF;
	...
	private void drawOBJLine(int yScreen, int priority) {
		...
		for (int objNumber = 127; objNumber >= 0; objNumber--) {
			...
			if (objPriority == priority) {
				...
				if (is1DMapping) {
					tileNumberIncrement = (is256ColorPalette
							? xTiles * 2
							: xTiles);
				} else {
				...
				if (!isRotScalEnabled) {
					...
					if (isDisplayable) {
						...
						if (yCoordinate >= YScreenSize)
							yCoordinate -= 256;

						if ((yScreen >= yCoordinate)
								&& (yScreen < yCoordinate + ySize)) {
							...
							for (int xSprite = 0; xSprite < xSize; xSprite++) {
								...
								if ((xScreen >= 0) && (xScreen < XScreenSize)) {
									...
									if (is256ColorPalette) {
										int tileNumber = firstTileNumber
												+ (yTile * tileNumberIncrement)
												+ (xTile * 2);
										...
										if (colorIndex != 0) {
											short rgb15 = palMem.getHalfWord(
													palBase + (colorIndex * 2));
											pixels[(yScreen * XScreenSize)
													+ xScreen] = toRGBA(rgb15);
										}
									} else {
										int tileNumber = firstTileNumber
												+ (yTile * tileNumberIncrement)
										...
										int colorIndex = vidMem.getByte(vidBase
												+ (tileNumber * 32)
												+ (tileY * 4) + (tileX / 2))
												& 0xFF;
