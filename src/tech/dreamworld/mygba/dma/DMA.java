package tech.dreamworld.mygba.dma;
...
public abstract class DMA {
	...
	private final static int ImmediateStartTiming = 0x0000,
			VBlankStartTiming = 0x1000, HBlankStartTiming = 0x2000,
	...
	private final void signal(int st) {
		if (isEnabled & (startTiming == st)) {
			...
			if (is32BitTransfer) {
				...
				for (int i = 0; i < dmaCount; i++) {
					memory.storeHalfWord(destination,
							memory.loadHalfWord(source));
					destination += dstAdd;
					source += srcAdd;
				}
			}
			if (dstControl == 0x0060)
				destination = old_destination;

			if (isIRQEnabled)
				iorMem.generateInterrupt(dmaInterruptBit);
